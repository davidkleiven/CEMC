...
void CHGLRealSpace<dim>::build2D(){
    ...
    MMSP::grid<2, grid> indexGrid(1, 0, L, 0, L);
    ...
    for (unsigned int i=0;i<MMSP::nodes(*this->grid_ptr);i++){
        ...
        for (int j=-1;j<2;j+=2){
            MMSP::vector<int> pos = grid_ptr->position(i);
            pos[dir] += j;
            unsigned int col = indexGrid(pos);
            matrix.insert(i, indx, -8*factor);
        }
    ...
    for (unsigned int field=1;field<dim+1;field++){
        ...
        for (unsigned int i=0;i<MMSP::nodes(*this->grid_ptr);i++){
            matrix.insert(i, i, 1.0 + 2*factor*(interface[field-1][0] + interface[field-1][1]));
            ...
            for (int ix=-1;ix<2;ix+=2){
                ...
                pos[dir] += ix;
                unsigned int col = indexGrid(pos);
                matrix.insert(i, col, -factor*interface[field-1][dir]);
            }
        }
    }
}

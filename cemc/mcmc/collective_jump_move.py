...
class CollectiveJumpMove(object):
    def __init__(self, mc_cell=None, a=4.05):
        """
        Extract the indices of that atoms in each column that can perform a
        ...
        Acta materialia 49.2 (2001): 321-328.
        """
        if mc_cell is None:
    ...
    def _extract_indices_in_column(self):
        ...
        """
        # The atoms that can perform jump moves are located at
        # (n * a * (1 + a/2), n * a * (1 + 1/2), z)
    ...
    def columns_are_valid(self):
        ...
        They need to have a 50 percent vacancy content
        """
        for (key, value) in self.columns.items():
            ...
            if first_has_vac:
                ...
                msg = "Vacancies are not ordered. {}".format(all_symbs)
                raise ValueError(msg)
    ...
    def system_changes_random_jump_move(self):
        ...
        for num in range(len(indices)):
            if num % 2 == 0:
                ...
                change = (indx, self.mc_cell[indices[num]],
                          self.mc_cell[indices[num + 1]])
            else:

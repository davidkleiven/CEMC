...
class CanonicalNucleationMC( Montecarlo ):
    ...
    def add_additional_atoms_to_match_concentration(self):
        """
        After there might be too few solute atoms after the initial cluster have been generated
        """
        ...
        for i in range(0,len(sorted_natoms)-1):
            ...
            if ( diff < 0 ):
                msg = "The system contains two many solute atoms. That should not be possible!\n"
                msg += "Failing for atom {}. Number present: {}. Concentration requires: {}".format(symb,at_count,self.n_atoms[symb])
                raise RuntimeError( msg )
            ...
            if ( num_trials >= max_trial_attemps ):
                raise RuntimeError( "Did not manage to create a system with the correct composition!" )
        ...
        for key in count.keys():
            if ( count[key] != self.n_atoms[key] ):
                ...
                msg += "Current atoms count: {}. Required atoms count: {}".format(count,self.n_atoms)
                raise RuntimeError( msg )

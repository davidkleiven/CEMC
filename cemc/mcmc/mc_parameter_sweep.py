...
class MCParameterSweep(object):
    ...
    def __init__( self, parameters, mc_obj, nsteps=100000, data_getter=None, outfile="default_output.h5", equil_params=None ):
    ...
    def check_initialization( self ):
        ...
        """
        if ( not self.mc_obj.name in self.known_parameters.keys() ):
            raise ValueError( "The Monte Carlo instance was not recognized. Known MonteCarlo object: {}".format(self.known_parameters.keys()) )
        ...
        for i in range(len(self.parameters)):
            for req_param in required_params:
                if ( req_param not in self.parameters[i].keys() ):
                    raise ValueError( "Required parameter {} not given for entry {}.".format(req_param,i) )

            # Check the format of the different parameters
            if ( "chemical_potential" in self.parameters[i].keys() ):
                if ( not isinstance(self.parameters[i]["chemical_potential"],dict) ):
                    raise ValueError( "Chemical potential has to be given as a dictionary" )
    ...
    def run( self ):
        ...
        for i in range(n_params):
            ...
            if ( name == "SGCMonteCarlo" ):
                ...
                self.mc_obj.chemical_potential = self.parameters[i]["chemical_potential"]
            else:
                raise NotImplementedError( "Parameter sweep for the MC object is not yet supported!" )
            self.mc_obj.runMC( steps=self.nsteps, equil=True, equil_params=self.equil_params )

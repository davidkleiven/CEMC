...
class SGCCompositionFreeEnergy(object):
    """
    Class that holds the nessecary information required to sample the DOS
    """
    def __init__( self, nbins=100, hist_limits=[] ):
        self.nbins = nbins
        self.dim_names = [hist_lim[0] for hist_lim in hist_limits]
        self.hist_limits = [(hist_lim[1],hist_lim[2]) for hist_lim in hist_limits]
    ...
    def get_one_index( self, value, dim=0 ):
        ...
        """
        indx = (value-self.hist_limits[dim][0])*self.nbins/(self.hist_limits[dim][1]-self.hist_limits[dim][0])
        return int(indx)
    ...
    def get_indx( self, singlets ):
        ...
        """
        indices = [self.get_one_index(singlets[dim],dim=dim) for dim in range(len(self.hist_limits))]
        return indices
    ...
    def find_dos( self, sgc_sampler, max_rel_unc=0.01, min_num_steps=0, rettype="comp_dos_obj" ):
        """
        Run Monte Carlo
        """
        ...
        if ( rettype not in supported_ret_types ):
            raise ValueError( "Return type has to be one of {}".format(supported_ret_types))
        while( counter < maxiter ):
            ...
            if ( counter%check_conv_every == 0 ):
                ...
                if ( max_std < max_rel_unc and counter > min_num_steps ):
                    print ("Composition Free Energy converged!. Max std: {}".format(max_std))
            ...
            if ( time.time()-last_time > output_every ):
                print ("Current number of steps: {}. Max std: {}.".format(counter,max_std))
                last_time = time.time()

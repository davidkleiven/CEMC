...
class TwoPhaseLandauPolynomial(object):
    ...
    def fit(self, conc1, free_energy1, conc2, free_energy2, tol=1E-4, penalty=1E-8,
        ...
        converged = False
        old_coeff = None
        self.optimal_shape_var = [np.random.rand(3) for _ in range(len(conc2))]
        best_coeff = None
        ...
        while not converged:
            ...
            X = np.zeros((len(conc2), len(terms)+1))
            X[:, 0] = [(conc2[i] - self.c2)*np.sum(shape**2)
    ...
    def optimize_shape_variables(self, conc):
        ...
        for i, c in enumerate(conc):
            def optimize_cost(x):
                return self.evaluate(c, x)

            res = minimize(optimize_cost, x0=self.optimal_shape_var[i],
                           bounds=[(0, 1), (0, 1), (0, 1)])
            self.optimal_shape_var[i] = res["x"]
